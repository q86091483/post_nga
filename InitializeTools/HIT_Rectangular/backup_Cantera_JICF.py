...
njet   = 2
P      = 10.0 * ct.one_atm
D_j    = 1.0E-3; A_j = 0.25 * np.pi * D_j * D_j
intv   = 1.0; 
Lx     = 12 * D_j;          
Ly     = (2*intv+2) * D_j; 
Lz     = 5.6 * D_j; 
...
T_j    = 400.;
T_c    = 750.;
X_j    = {}; X_j["H2"] = 0.65; X_j["N2"] = 1 - X_j["H2"] 
...
print("Domain x:    {:5.2E} - {:5.2E}".format(0, Lx))
...
zc = H * 0.45 * np.power(xc/H, 0.25) * np.exp(-b)
xw = np.argmax(x_>jet_xs[0]-D_j/0.25)
wp = H * 0.7 * np.power(xc/H, 0.25)
wn = H * 0.25 * np.power(xc/H, 0.25)
