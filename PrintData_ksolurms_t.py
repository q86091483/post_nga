...
if __name__ == '__main__':
  ...
  if (myid == 0):
    ...
    axres.plot(np.asarray(res_t) / tau, res_urms/u0s, label='urms')
    axres.plot(np.asarray(res_t) / tau, res_vrms/u0s, label='vrms')
    axres.plot(np.asarray(res_t) / tau, res_wrms/u0s, label='wrms') 
    k0s = 13.5*13.5*A*A*(0.18*6.28)**2
    axres.plot([0, res_t[-1]/tau], [1, 1], 'b--') 
    axres.legend()
    ...
    plt.savefig(os.path.join(resfigs_case_folder, "ksolurms_t.png"), bbox_inches="tight")
